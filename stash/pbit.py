...
def convert_pbit_to_json(pbit_file_path, output_json_path):
    # Step 1: Rename the .pbit file to .zip and extract it
    zip_file_path = pbit_file_path.replace(".pbit", ".zip")
    os.rename(pbit_file_path, zip_file_path)
    ...
    if data_model_schema_path is None:
        raise FileNotFoundError("DataModelSchema not found in the extracted .pbit file")
...
# Example usage
pbit_file_path = "reportqor.pbit"  # Replace with the actual .pbit file path
output_json_path = "output_template.json"  # Replace with the desired output JSON path

convert_pbit_to_json(pbit_file_path, output_json_path)
